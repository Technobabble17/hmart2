<x-app-layout>
    <x-slot name="header">
        ...
        <a class="text-blue-600 border-b-2 border-transparent hover:border-blue-600 transition duration-300 ease-in-out" href="{{ route('user.update') }}">Edit Account Details</a>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8 bg-white overflow-hidden shadow-2xl sm:rounded-lg">
                <article class="p-6 bg-gray-300 border-b border-gray-200">
                    <h2 class="text-2xl bold pb-4">{{ $user->name }}</h2>
                    <p>{{ $user->about }}</p>
                    <p>{{ $user->email }}</p>
                    <p>{{ $user->phone }}</p>
                    <p>{{ $user->paypal }}</p>
